...
public class Histogram3D {
	 ...
	 */
	public Histogram3D(String name, int bx, double xMin, double xMax, int by,
	 ...
	 */
	public Histogram3D getRegion(String name, int bx_start, int bx_end,
		...
		Histogram3D regHist = new Histogram3D(name, bx_end - bx_start, newXMin, newXMax,
	...
	public Histogram2D sliceXY(int zBin) {
		...
		double yMin = yAxis.min();
		double yMax = yAxis.max();
		Histogram2D xy = new Histogram2D(name, xNum, xMin, xMax, yNum, yMin, yMax);
	...
	public Histogram2D sliceXZ(int yBin) {
		...
		double zMin = zAxis.min();
		double zMax = zAxis.max();
		Histogram2D xz = new Histogram2D(name, xNum, xMin, xMax, zNum, zMin, zMax);
	 ...
	 */
	public Histogram2D sliceYZ(int xBin) {
		if (xBin >= 0 && xBin < xAxis.getNBins()) {
			...
			double zMin = zAxis.min();
			double zMax = zAxis.max();
			Histogram2D yz = new Histogram2D(name, yNum, yMin, yMax, zNum, zMin, zMax);
